function lastLevel() {
    ...
    var BlockSet = [
        ...
        { antallBlokker: 25, xPos: canvasEl.width * Math.random(), yPos: canvasEl.height * Math.random(), damage: 0, jump: 8.5, speed: 0, name: "jump" },
...
class blocks {
    ...
    */
    ...
    for (i = 0; i < CarrotSet.length; i++) {
        carrots[i] = new Array();

        for (var j = 0; j < CarrotSet[i].antall; j++) {
            carrots[i][j] = new collectable("img/treasure.png", CarrotSet[i].xPos + (j * 400), CarrotSet[i].yPos, 50, 80)
        }
    }
    //
